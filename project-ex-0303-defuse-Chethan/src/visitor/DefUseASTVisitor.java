...
public class DefUseASTVisitor extends ASTVisitor {
   ...
   @Override
	...
	public boolean visit(MethodDeclaration node) {
		if (node.getName().getFullyQualifiedName().equals(METHOD_NAME)) {
			return true;
		}
		return false;
	}
	...
	/*
	 * @Override public boolean visit(VariableDeclarationStatement node) { for
	 * (Iterator<?> iter = node.fragments().iterator(); iter.hasNext();) {
	 * VariableDeclarationFragment f = (VariableDeclarationFragment) iter.next();
	 * 
	 * IVariableBinding b = f.resolveBinding(); DefUseModel data = new
	 * DefUseModel(node, f, this.compilationUnit); defUseMap.put(b, data); } return
	 * super.visit(node); }
	 */
