...
/*public class DefUseASTVisitor extends ASTVisitor {
	   ...
	   if (node.getName().getIdentifier().equals(this.callee)) {
	         ...
	         node.accept(new ASTVisitor(){
        		 public boolean visit(IfStatement IfStat) {
        			 IfStat.accept(new DefUseAnalASTVisitor("if-statement"));
        			 return true;
        		 }
        		 public boolean visit(ForStatement ForStat) {
        			 ForStat.accept(new DefUseAnalASTVisitor("for-statement"));
        			 return true;
        		 }
        	 });
  ...
  /* class DefUseAnalASTVisitor extends ASTVisitor{
	   private String ctrlBody;
	private CompilationUnit compilationUnit;
	   
	   public DefUseAnalASTVisitor(String ctrlBody) {
		   this.ctrlBody=ctrlBody;
	   }  
   ...
   public boolean visit(VariableDeclarationStatement node) {
	       for (Iterator<?> iter = node.fragments().iterator(); iter.hasNext();) {
    	         VariableDeclarationFragment varFrag= (VariableDeclarationFragment) iter.next();

         IVariableBinding varBind = varFrag.resolveBinding();
         DefUseModel data = new DefUseModel(node, varFrag, this.compilationUnit);
         defUseMap.put(varBind, data);
 ...
 * @since JavaSE-1.8
 */
public class DefUseASTVisitor2 extends ASTVisitor {
   private Map<IVariableBinding, DefUseModel> defUseMap = new HashMap<IVariableBinding, DefUseModel>();
   private int fAccessesToSystemFields;
   private CompilationUnit compilationUnit;
  //  private int callType; 
   ...
   public DefUseASTVisitor2(CompilationUnit compilationUnit, String callee, IPackageFragment[] p) {
	   this.packages = p;
      this.compilationUnit = compilationUnit;
      this.callee = callee;
   }
